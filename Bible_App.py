...
# --- SAFE CACHING FUNCTION (PREVENTS CRASHES) ---
@st.cache_data
def load_v10_data_cached():
    """Safely loads V10 database, handling both old (4-item) and new (6-item) formats."""
    data = v10_engine.build_team_database()
    
    # SAFE UNPACKING LOGIC
    if len(data) == 6:
        return data # Returns: stats, style, quad, eff, h_perf, r_perf
    elif len(data) == 4:
        # Fallback for old simulator version
        stats, style, quad, eff = data
        return stats, style, quad, eff, None, None
    else:
...
def load_all_data():
    ...
    try:
        ...
        if missing: data['kp'] = pd.DataFrame(); return data
...
def run_hypothetical(visitor, home, neutral, data):
    ...
    return {'V_Score': mean_v, 'H_Score': mean_h, 'Spread': mean_h - mean_v, 'Total': mean_v + mean_h, 'Home_Win_Prob': 50 + (mean_h - mean_v)*2.5}
...
if st.sidebar.button("🔄 Refresh Data", type="primary"): st.cache_data.clear(); st.rerun()
...
if mode == "🔍 Scout Team":
    ...
    if not kp.empty:
        ...
        if team:
            ...
            c1.metric("Rank", row.get("Rank", 0)); c2.metric("AdjEM", f"{row.get('AdjEM', 0):.1f}")
            c3.metric("Off Eff", f"{row.get('AdjOE', 0):.1f}"); c4.metric("Luck", f"{row.get('Luck', 0):.3f}")
    ...
    if not kp.empty:
        ...
        m1, m2 = st.columns(2)
        market_spread = m1.number_input("Market Spread", value=None, step=0.5, format="%.1f")
            ...
            if visitor and home and visitor != home:
                ...
                if stats is None:
                    ...
                    with col_base:
                        ...
                        if results_v92:
                            st.markdown(f"""<div class="sim-result-box" style="border-color: #94a3b8;"><div style="font-size: 12px; color: #666;">BASELINE PROJECTION</div><div class="score-display">{visitor} {results_v92['V_Score']:.1f}<br>{home} {results_v92['H_Score']:.1f}</div><div class="spread-display">Line: {home} {-(results_v92['Spread']):+.1f} | Total: {results_v92['Total']:.1f}</div></div>""", unsafe_allow_html=True)
                    with col_exp:
                        st.subheader("🧪 V10 (Production)")
                        v10_disp_line = -results_v10['Predicted_Spread']
                        st.markdown(f"""<div class="sim-result-box" style="border-color: #4f46e5; background-color: #eef2ff;"><div style="font-size: 12px; color: #4338ca;">PhD-LEVEL ADJUSTED</div><div class="score-display" style="color: #312e81;">{results_v10['Visitor']} {results_v10['V_Score']}<br>{results_v10['Home']} {results_v10['H_Score']}</div><div class="spread-display" style="color: #3730a3;">Line: {results_v10['Home']} {v10_disp_line:+.1f}</div></div>""", unsafe_allow_html=True)
                        st.write("")
